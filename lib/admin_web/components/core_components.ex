defmodule AdminWeb.CoreComponents do
  ...
  def dumper_link(assigns) do
    base = ~p"/dashboard/dumper"

    src =
      case assigns do
        %{record: %mod{id: id}} -> base <> "?module=#{mod}&id=#{id}"
        %{module: mod, id: id} -> base <> "?module=#{mod}&id=#{id}"
        %{module: mod} -> base <> "?module=#{mod}"
        _ -> base
      end

    assigns = assign(assigns, src: src)
    ~H|<.link href={@src} class="underline">{render_slot(@inner_block)}</.link>|
  end
